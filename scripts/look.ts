...
for (const v of views) {
  const scene = renderScene(voxels, { view: v as View, maxPixels: +(process.env.MAXPX ?? 1100), hide: (process.env.HIDE ?? '').split(',').filter(Boolean), cutawayY: process.env.CUT ? +process.env.CUT : undefined });
